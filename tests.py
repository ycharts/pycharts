...
class MockHttpResponse(object):
	...
	URL_RESPONSE_INDEX = {
		...
		{'response': {'AAPL': {'meta': {'status': 'ok'}, 
		'results': {'price': {'meta': {'status': 'ok'}, 
		'data': ['2016-09-15', 115.39]}}}, 'MSFT': {'meta': {'status': 'ok'}, 
		'results': {'price': {'meta': {'status': 'ok'}, 'data': ['2016-09-15', 57.21]}}}}, 
		...
		{'response': {'AAPL': {'results': {'price': 
		{'data': [['2016-09-12', 105.44], ['2016-09-13', 107.95], ['2016-09-14', 111.77],
		['2016-09-15', 115.39]], 'meta': {'status': 'ok'}}}, 'meta': {'status': 'ok'}}}, 
		'meta': {
		'url': 'http://ycharts.com/api/v3/companies/AAPL/series/price?start_date=2016-09-10', 
		'status': 'ok'}},
		'https://ycharts.com/api/v3/companies/AAPL/info/name': {'response': {'AAPL': {'results': {'name': {'data': 'Apple', 'meta': {'status': 'ok'}}}, 
		'meta': {'status': 'ok'}}}, 'meta': {'url': 'http://ycharts.com/api/v3/companies/AAPL/info/name', 
		'status': 'ok'}},
		'https://ycharts.com/api/v3/companies/TOOMANY/info/name': 
			{'response': {}, 'meta': 
			{'error_message': 'Too many identifiers. Ensure 100 or less.', 'status': 'error', 'error_code': 414,
	...
	def read(self):
		return json.dumps(self.URL_RESPONSE_INDEX[self.request.get_full_url()]).encode()
...
class ClientTestCase(TestCase):
	...
	def test_400_raise(self):
		try:
			point_rsp = self.client.get_points(['AAPL'], ['price'], query_date=45)
			# it should not get here!
			self.assertEqual(True, False)
		except exceptions.PyChartsRequestException as date_exception:
			self.assertEqual(date_exception.error_code, 400)

	@mock.patch('pycharts.base.urlopen', mock_urlopen)
	def test_414_raise(self):
		try:
			info_rsp = self.client.get_info(['TOOMANY'], ['name'])
			# it should not get here!
			self.assertEqual(True, False)
		except exceptions.PyChartsRequestTooLongException as too_long_exception:
			self.assertEqual(too_long_exception.error_code, 414)
